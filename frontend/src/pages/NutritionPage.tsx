...
export default function NutritionPage() {
  ...
  useEffect(() => {
    document.title = 'Nutrition - Forge';
  }, []);
  useEffect(() => {
    loadData();
  }, [date]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSummary((prev) =>
      prev
        ? {
            ...prev,
            meals: [...prev.meals, optimistic],
            totalCalories: prev.totalCalories + optimistic.calories,
            totalProtein: prev.totalProtein + optimistic.proteinG,
            totalCarbs: prev.totalCarbs + optimistic.carbsG,
            totalFat: prev.totalFat + optimistic.fatG,
          }
        : prev,
    );
    setShowForm(false);
    ...
    try {
      ...
      if (res.success) toast.success('Food logged!');
      loadData();
    } catch {
  ...
  const renderMealGroup = (category: string) => {
    ...
    return (
      <div className="mb-5">
        <h4
          className="flex justify-between text-sm font-medium mb-2.5 pb-2"
            ...
            >
              <div>
                ...
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                  {m.proteinG}g P &middot; {m.carbsG}g C &middot; {m.fatG}g F
                </p>
              </div>
              <div className="flex items-center gap-2.5">
                <span className="text-sm font-medium" style={{ color: 'var(--primary-light)' }}>
                  {m.calories}
                </span>
                <button
                  onClick={() => handleDeleteLog(m.id)}
                  className="btn-danger"
                  style={{ padding: '3px 6px', borderRadius: '5px', fontSize: '11px' }}
                >
                  <Trash2 size={12} />
                </button>
              </div>
              ...
              >
                ...
                {searchResults.length > 0 && (
                  ...
                  >
                    ...
                    {searchResults
                      .slice(searchPage * RESULTS_PER_PAGE, (searchPage + 1) * RESULTS_PER_PAGE)
                      .map((p, i) => (
                        <button
                          key={i}
                          type="button"
                          ...
                          onMouseEnter={(e) => {
                            (e.currentTarget as HTMLElement).style.background =
                              'hsla(var(--hue-primary), 55%, 50%, 0.06)';
                          }}
                          onMouseLeave={(e) => {
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setFormData((prev) => ({ ...prev, servingSize: v }));
                    const n = parseInt(v) || 100;
                    if (Object.keys(basePer100g).length) applyServingScale(basePer100g, n);
                  }}
                />
              </div>
              <div>
            ...
            <div className="md:col-span-2 card">
              ...
              {summary.meals.length === 0 ? (
                ...
                <>
                  ...
                  {renderMealGroup('PRE_WORKOUT')}
                  {renderMealGroup('POST_WORKOUT')}
          ...
          {summary?.micronutrients && (
            <div className="card">
              <h3
                ...
                style={{ color: 'var(--text)' }}
              >
                <BarChart3 size={16} /> Micronutrients{' '}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
                {MICRO_FIELDS.filter(
                  (k) => (summary.micronutrients![k] || 0) > 0 || NUTRITION_RDA[k]?.rda > 0,
                ).map((key) => {
                  const info = NUTRITION_RDA[key];
                  if (!info) return null;
