...
export default function WorkoutsPage() {
  ...
  const [exerciseForm, setExerciseForm] = useState({
    exerciseName: '',
    setEntries: [{ weightKg: 0, reps: 10, rpe: '' }] as SetEntry[],
    duration: 0,
    zone: '',
    notes: '',
  });
  ...
  const loadPredictions = async (records: PersonalRecord[]) => {
    ...
    await Promise.all(
      records.map(async (pr) => {
        try {
          const res = await workoutApi.predict1RM(pr.exerciseName);
          if (res.success) results[pr.exerciseName] = res.data;
        } catch {}
      }),
    );
  ...
  const handlePredictOne = async (exerciseName: string) => {
    try {
      const res = await workoutApi.predict1RM(exerciseName);
      if (res.success) setPredictions((prev) => ({ ...prev, [exerciseName]: res.data }));
    } catch {
  ...
  const addSetRow = () =>
    setExerciseForm((prev) => ({
      ...
      setEntries: [
        ...prev.setEntries,
        { weightKg: 0, reps: prev.setEntries[0]?.reps || 10, rpe: '' },
      ],
    }));
  const removeSetRow = (idx: number) => {
    if (exerciseForm.setEntries.length <= 1) return;
    setExerciseForm((prev) => ({
      ...prev,
      setEntries: prev.setEntries.filter((_, i) => i !== idx),
    }));
  };
  ...
  const handleAddExercise = async (sessionId: number, e: React.FormEvent) => {
    ...
    if (category === 'STRENGTH') {
      ...
      if (filledSets.length === 0) {
        toast.error('Add at least one set with weight and reps');
        return;
      }
      ...
      const avgRpe =
        rpeVals.length > 0 ? Math.round(rpeVals.reduce((a, b) => a + b, 0) / rpeVals.length) : null;
      payload = { ...payload, sets: totalSets, reps: avgReps, weightKg: maxWeight, rpe: avgRpe };
    } else {
      if (!exerciseForm.duration || exerciseForm.duration <= 0) {
        toast.error('Enter duration in minutes');
        return;
      }
      if (!exerciseForm.zone) {
        toast.error('Select a heart rate zone');
        return;
      }
    ...
    setExerciseForm({
      exerciseName: '',
      setEntries: [{ weightKg: 0, reps: 10, rpe: '' }],
      duration: 0,
      zone: '',
      notes: '',
    });
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setSessionForm({ name: '', date: format(new Date(), 'yyyy-MM-dd') });
        loadData();
      }
    } catch {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            Last 30 days &middot; {sessions.length} sessions
          </p>
        </div>
        <motion.button
        ...
        <div className="md:col-span-2 flex flex-col gap-3">
          ...
          {loading ? (
            ...
            <div className="card" style={{ padding: '40px', textAlign: 'center' }}>
              <Dumbbell
                size={28}
                style={{ margin: '0 auto 10px', opacity: 0.3, color: 'var(--text-muted)' }}
              />
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            ...
            <AnimatePresence>
              {sessions.map((session, si) => (
                ...
                >
                  ...
                  >
                    <div>
                      <h4 className="text-sm font-medium" style={{ color: 'var(--primary-light)' }}>
                        {session.name}
                      </h4>
                      <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
                        {session.date} &middot; {session.durationMinutes || '-'} min
                      </span>
                    </div>
                    <span className="badge badge-purple">
                      {session.totalVolume.toFixed(0)} kg vol.
                    </span>
                  </div>
                  <div style={{ padding: '12px 16px' }}>
                    {session.exercises && session.exercises.length > 0 ? (
                      ...
                      >
                        <thead>
                          ...
                          >
                            <th
                              style={{ textAlign: 'left', paddingBottom: '6px', fontWeight: 500 }}
                            >
                              Exercise
                            </th>
                            <th
                        ...
                        <tbody>
                          {session.exercises.map((ex) => {
                            ...
                            const epley1RM =
                              isStrength && ex.reps > 0
                                ? ex.weightKg * (1 + ex.reps / 30)
                                : ex.weightKg;
                            return (
                              <tr key={ex.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                ...
                                <td style={{ padding: '7px 0', color: 'var(--text-secondary)' }}>
                                  {isStrength
                                    ? `${ex.sets} x ${ex.reps}`
                                    : `${ex.duration}min Z${ex.zone?.replace('ZONE_', '')}`}
                                </td>
                                <td
                                  style={{
                                    padding: '7px 0',
                                ...
                                >
                                  {isStrength ? `${epley1RM.toFixed(0)} kg` : '-'}
                                </td>
                              </tr>
                            );
                          })}
                    ...
                    <motion.button
                      whileHover={{ x: 2 }}
                      onClick={() =>
                        setAddingExerciseForSession(
                          addingExerciseForSession === session.id ? null : session.id,
                        )
                      }
                      style={{
                        marginTop: '10px',
                        ...
                        fontSize: '12px',
                        color: 'var(--primary-light)',
                        background: 'hsla(var(--hue-primary), 55%, 50%, 0.06)',
                        border: '1px solid hsla(var(--hue-primary), 55%, 50%, 0.12)',
                        padding: '5px 10px',
                        borderRadius: '7px',
                        cursor: 'pointer',
                      }}
                    ...
                    <AnimatePresence>
                      {addingExerciseForSession === session.id && (
                        <motion.form
                          ...
                          onSubmit={(e) => handleAddExercise(session.id, e)}
                          style={{
                            marginTop: '10px',
                            padding: '12px',
                        ...
                        >
                              ...
                              {(['STRENGTH', 'CARDIO'] as const).map((c) => (
                                <button
                                  ...
                                  style={{
                                    ...
                                    background:
                                      category === c
                                        ? 'hsla(var(--hue-primary), 55%, 50%, 0.08)'
                                        : 'rgba(255,255,255,0.02)',
                                    color:
                                      category === c ? 'var(--primary-light)' : 'var(--text-muted)',
                                  }}
                                >
                          ...
                          {category === 'STRENGTH' ? (
                                ...
                                {exerciseForm.setEntries.map((entry, idx) => (
                                  ...
                                  >
                                    ...
                                    <input
                                      ...
                                      onChange={(e) =>
                                        updateSetRow(idx, 'weightKg', e.target.value)
                                      }
                                    />
                                    <span
                                      className="text-xs"
                                    ...
                                    <input
                                      ...
                                      placeholder="RPE"
                                      value={entry.rpe}
                                      onChange={(e) => updateSetRow(idx, 'rpe', e.target.value)}
                                    />
                            ...
                            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                              ...
                              <div style={{ flex: 1 }}>
                                ...
                                <select
                                  className="input"
                                  value={exerciseForm.zone}
                                  onChange={(e) =>
                                    setExerciseForm((prev) => ({ ...prev, zone: e.target.value }))
                                  }
                                >
                          ...
                          <div className="flex justify-end gap-2">
                            <button
                              type="button"
                              onClick={() => setAddingExerciseForSession(null)}
                              className="btn btn-secondary"
                              style={{ padding: '6px 12px', fontSize: '12px', borderRadius: '7px' }}
                            >
                              Cancel
                            </button>
                            <button
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div className="card" style={{ padding: '16px' }}>
            {loading || predicting ? (
              ...
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                Log workouts to generate PRs and predictions.
              </p>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
                          1RM ~{pr.estimatedOneRepMax.toFixed(0)}
                        </span>
                      </div>
                      {pred ? (
                        <div className="flex gap-1.5 flex-wrap">
                          <span className="badge badge-green">
                            +30d: {pred.predicted30Days1RM} kg
                          </span>
                          <span className="badge badge-green">
                            +60d: {pred.predicted60Days1RM} kg
                          </span>
                          <span className="badge badge-green">
                            +90d: {pred.predicted90Days1RM} kg
                          </span>
                        </div>
                      ) : (
                        <button
                          onClick={() => handlePredictOne(pr.exerciseName)}
                          style={{
                            fontSize: '11px',
                            padding: '2px 8px',
                            borderRadius: '6px',
                            background: 'hsla(var(--hue-primary), 55%, 50%, 0.08)',
                            color: 'var(--primary-light)',
                            border: '1px solid hsla(var(--hue-primary), 55%, 50%, 0.12)',
                            cursor: 'pointer',
                            display: 'inline-flex',
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between mb-1">
                        ...
                        <span style={{ color }}>
                          {p.current1RM} kg &rarr; {p.predicted90Days1RM} kg
                        </span>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{
                            width: `${Math.min(100, (p.predicted90Days1RM / p.current1RM) * 100)}%`,
                          }}
                          style={{
                            height: '100%',
                            background: color,
